...
def build_system_prompt(include_hours: bool = True) -> str:
    ...
    if not include_hours:
        return (
            ...
            "2. Leave `sources` empty and `grounded=false` for every task — there is "
            "no historical context here. Do not invent citations.\n"
    ...
    return (
        ...
        "2. For EVERY task, set `grounded=true` only when a <source> block directly "
        "supports it; otherwise set `grounded=false`, leave `sources` empty and do NOT "
        "invent engineer-days for that task.\n"
        "3. When `grounded=true`, populate `sources` with one or more SourceReference "
        "objects per task:\n"
        "   - `chunk_id`: the `id` attribute of the <source> element (never invented).\n"
        "   - `document_id`: the `document_id` attribute of that <source>.\n"
        "   - `evidence`: copy the verbatim span or figure from the source content that "
        "backs the task — do NOT paraphrase.\n"
        "4. Genuinely novel scope with no historical analog must be expressed as an "
        "Assumption (not as a grounded task citing a non-existent id).\n"
        "5. Clearly distinguish evidence-backed tasks (`grounded=true` with sources) "
        "from assumptions and ungrounded tasks.\n"
        "6. total_engineer_days must equal the sum of all grounded tasks across all "
        "modules.\n"
        "7. If the provided context is insufficient to estimate responsibly, set "
        ...
        "8. Otherwise set confidence to high/medium/low based on how well the sources "
...
def build_user_message(context_block: str, structured_query: EstimationQuery) -> str:
    ...
    return (
        ...
        "</sources>\n"
        "\n"
        "Produce the grounded estimate now. For each task cite chunk_id, document_id "
        "and verbatim evidence from the <sources> block, or mark grounded=false."
    )
