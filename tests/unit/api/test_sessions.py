...
def test_get_session_debug_exposes_tier_after_resolution(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.api.sessions.settings.TIER_RESOLUTION_ENABLED", True)
...
def test_session_estimate_emits_turn_observed(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.api.sessions.log.info", fake_log_info)
...
def test_session_estimate_rejects_empty_description_without_attachments(
    ...
    monkeypatch.setattr(
        "app.api.sessions.generate_estimation", fail_generate_estimation
    )
