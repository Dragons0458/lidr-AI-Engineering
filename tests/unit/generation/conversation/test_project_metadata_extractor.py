...
from app.generation.conversation.metadata_extractor import (
...
def test_extract_project_metadata_uses_latest_metadata_state(
    ...
    monkeypatch.setattr(
        "app.generation.conversation.metadata_extractor._create_project_metadata_completion",
...
def test_create_project_metadata_completion_uses_primary_model(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.generation.conversation.metadata_extractor._client.chat.completions.create",
...
def test_instructor_client_uses_llm_wrapper_completion() -> None:
    from app.foundation.llm.wrapper import completion as wrapper_completion

    import app.generation.conversation.metadata_extractor as extractor

    assert extractor._client is not None
    assert wrapper_completion.__name__ == "completion"
    assert wrapper_completion.__module__ == "app.foundation.llm.wrapper"
