...
_FM_DIR = (
    Path(__file__).resolve().parents[5] / "exercises" / "session-14" / "failure_modes"
)
...
def test_broken_router_ping_pongs_until_step_budget():
    routing = _load("routing_no_converge")
    history = routing.run_router(
        ["requirements_extractor", "budget_searcher"], guard=False
    )
...
def test_fixed_router_converges_without_budget_limit():
    routing = _load("routing_no_converge")
    history = routing.run_router(
        ["requirements_extractor", "budget_searcher"], guard=True
    )
    assert history[-1]["source"] != "limit"
...
def test_plain_channel_rejects_concurrent_write():
...
@pytest.mark.asyncio
async def test_mismatched_thread_id_leaves_run_paused():
...
@pytest.mark.asyncio
async def test_matching_thread_id_resumes_same_run():
    mod = _load("interrupt_no_resume")
    graph = mod.build_gate_graph()
    report = await mod.start_and_resume(
        graph,
        "est-1",
        start_thread="s14:est-1",
        resume_thread="s14:est-1",
        decision="approve",
    )
